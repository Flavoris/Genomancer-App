#!/usr/bin/env bash
# chmod +x "Gene Whisperer/scripts/colab_run_stage2.sh"
#
# Colab Stage 2 Training Script for Gene Whisperer
# Runs Stage 2 classifier training (strong vs weak) with robust checkpointing and Drive backup.
#
# Usage (in Colab):
#   !bash "Gene Whisperer/scripts/colab_run_stage2.sh"
#   !bash "Gene Whisperer/scripts/colab_run_stage2.sh" --run_name my_experiment
#   !bash "Gene Whisperer/scripts/colab_run_stage2.sh" --kmer 4 --drive_dir /content/drive/MyDrive/CustomDir
#   !bash "Gene Whisperer/scripts/colab_run_stage2.sh" --stage1_ckpt path/to/stage1_k4.pt
#
# This script:
#   1. Checks Google Drive is mounted
#   2. Runs colab_bootstrap.sh to set up environment
#   3. Creates output directories on Drive
#   4. Runs Stage 2 training with logging to Drive
...
DEFAULT_RUN_NAME="stage2_$(date +%Y%m%d_%H%M%S)"
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            ...
            echo "  --run_name NAME     Run name for output directory (default: stage2_YYYYMMDD_HHMMSS)"
            echo "  --stage1_ckpt PATH  Optional Stage 1 checkpoint to initialize from"
...
# =============================================================================
echo "============================================================"
echo "Gene Whisperer - Colab Stage 2 Training"
echo "============================================================"
echo ""
...
if [[ -d "$CHECKPOINTS_DIR" ]]; then
    ...
    LATEST_CHECKPOINT=$(find "$CHECKPOINTS_DIR" -name "stage2*.pt" -type f 2>/dev/null | sort -r | head -n 1 || true)
fi

if [[ -n "$LATEST_CHECKPOINT" ]]; then
    log_info "Found existing checkpoint: $LATEST_CHECKPOINT"
    log_warn "NOTE: train_stage2.py does not yet support --resume flag."
...
LOG_FILE="$LOGS_DIR/stage2_training_$(date +%Y%m%d_%H%M%S).log"

# Build the training command
TRAIN_CMD="python train_stage2.py --config \"$CONFIG_PATH\""
...
# Add stage1_ckpt argument if specified
if [[ -n "$STAGE1_CKPT" ]]; then
    log_info "Stage 1 checkpoint specified: $STAGE1_CKPT"
    # Resolve relative path if needed
    if [[ "$STAGE1_CKPT" != /* ]]; then
        STAGE1_CKPT="$REPO_ROOT/$STAGE1_CKPT"
    fi
    if [[ -f "$STAGE1_CKPT" ]]; then
        log_info "Using Stage 1 checkpoint: $STAGE1_CKPT"
        TRAIN_CMD="$TRAIN_CMD --stage1_ckpt \"$STAGE1_CKPT\""
    else
        log_warn "Stage 1 checkpoint not found: $STAGE1_CKPT"
    fi
fi
...
cleanup_and_copy() {
    ...
    # Copy any stage2_report*.json files from artifacts
    log_info "Copying stage2_report*.json files..."
    find "$LOCAL_ARTIFACTS" -name "stage2_report*.json" -type f -exec cp -v {} "$ARTIFACTS_DIR/" \; 2>&1 || true

    # Copy from runs directory if it exists
    RUNS_DIR="$REPO_ROOT/Gene Whisperer/runs"
    if [[ -d "$RUNS_DIR" ]]; then
        log_info "Copying from runs directory..."
        # Find the most recent stage2 run
        LATEST_RUN=$(find "$RUNS_DIR" -maxdepth 1 -type d -name "stage2_*" 2>/dev/null | sort -r | head -n 1 || true)
    ...
    # Copy stage2 checkpoints specifically
    if [[ -d "$LOCAL_ARTIFACTS/checkpoints" ]]; then
        find "$LOCAL_ARTIFACTS/checkpoints" -name "stage2*.pt" -type f -exec cp -v {} "$CHECKPOINTS_DIR/" \; 2>&1 || true
    fi
...
# =============================================================================
log_info "============================================================"
log_info "Starting Stage 2 Training"
log_info "============================================================"
echo ""
