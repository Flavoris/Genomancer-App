...
def _apply_quick_defaults(cfg: dict[str, Any]) -> None:
    if cfg.get("ablation_max_train_samples_stage2") is None:
        cfg["ablation_max_train_samples_stage2"] = 2048
    if cfg.get("ablation_max_val_samples_stage2") is None:
        cfg["ablation_max_val_samples_stage2"] = 2048
    if cfg.get("ablation_max_steps_stage2") is None:
        cfg["ablation_max_steps_stage2"] = 200
...
def _run_variant(
    *,
    ...
    for seed in seeds:
        ...
        report = run_stage2_training(base_cfg, overrides=overrides)
        try:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 2 ablation runner")
    ...
    from train_stage2 import load_config, run_stage2_training  # type: ignore
    ...
    out_path = artifacts_root / "ablation_stage2.csv"
    ...
    stage1_csv_path = artifacts_root / "ablation_stage1.csv"
    stage1_candidates = (
    ...
    overall_candidates = (
        sorted(set(stage1_candidates).intersection(candidates_to_remove)) if stage1_candidates is not None else None
    )
    if overall_candidates is not None:
        ...
        for name in overall_candidates:
            overall_overrides.update(stage1_shared_overrides.get(name, {}))
        ...
        with overall_override_path.open("w", encoding="utf-8") as handle:
            handle.write("# Generated by ablation_stage2.py\n")
    ...
    print("\n" + "=" * 80)
    print("STAGE 2 ABLATION SUMMARY")
    print("=" * 80)
    ...
    if candidates_to_remove:
        print("Candidates to remove (stage 2):")
        for row in negligible_rows:
            print(f"- {row.name} (ΔAUC {row.delta_auc_mean:+.4f}, ΔMCC {row.delta_mcc_mean:+.4f})")
    else:
        print("Candidates to remove (stage 2): <none>")
    if stage1_candidates is None:
        print("Overall candidates (stage 1 + stage 2): <stage 1 CSV not found; run ablation_stage1.py first>")
    else:
        print("Overall candidates (stage 1 + stage 2): " + (", ".join(overall_candidates) if overall_candidates else "<none>"))
